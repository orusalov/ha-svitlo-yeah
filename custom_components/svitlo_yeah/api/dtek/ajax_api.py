"""Base for DTEK live (cookie-based) API implementations."""
...
def _extract_raw_csrf_token(csrf_cookie_value: str, log_prefix: str) -> str | None:
    """Extract raw CSRF token from Yii2 _csrf cookie value.

    Cookie format (URL-encoded PHP serialized):
    hash:2:{i:0;s:N:"_csrf-dtek-xxx";i:1;s:32:"RAW_TOKEN";}
    """
    decoded = unquote(csrf_cookie_value)
    match = _CSRF_TOKEN_RE.search(decoded)
    if match:
        return match.group(1)
    LOGGER.warning("%s: Cannot extract raw token from CSRF cookie: %s", log_prefix, decoded)
    return None
...
class DtekAjaxAPIBase(DtekAPIBase):
    """Base for DTEK APIs that use cookie-based Yii2 CSRF authentication."""

    _csrf_cookie_name: str = ""
    _ajax_url: str = ""
    _shutdowns_url: str = ""
    _origin: str = ""
    _log_prefix: str = "DTEK"
    ...
    def _get_csrf_token(self) -> str | None:
        """Build a masked CSRF token from the stored _csrf cookie."""
        csrf_cookie = self._cookies.get(self._csrf_cookie_name)
        if not csrf_cookie:
            LOGGER.error("%s: %s cookie not found", self._log_prefix, self._csrf_cookie_name)
            return None
        raw_token = _extract_raw_csrf_token(csrf_cookie, self._log_prefix)
        if not raw_token:
            return None
    ...
    async def fetch_data(self) -> None:
        ...
        if not data.get("result"):
            LOGGER.debug(
                "%s: No update since %s — cached data still valid",
                self._log_prefix,
                self._last_update,
            )
            return
        ...
        LOGGER.debug("%s: Data updated, update=%s", self._log_prefix, self._last_update)
